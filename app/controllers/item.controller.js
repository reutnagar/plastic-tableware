...
function addItem(req,res) {
    ...
    var body = '';
        ...
        var newItem = new Item({ category : POST.category,subCategory :  POST.subCategory ,name : POST.name , description : POST.description,price: POST.price, location : POST.location, index: counter++ });
        newItem.save();           
        res.send(newItem);
...
function changeItem(req,res) {
	...
	var body = '';
        ...
        req.on('end', function () {
        var POST = qs.parse(body);
		Item.findOneAndUpdate({index : POST.index }, {$set: {category : POST.category,subCategory :  POST.subCategory , description : POST.description,price: POST.price ,location : POST.location, name : POST.name }}, function(err, doc){
		if(err){
            throw err;
        }
